...
### How to use it
The main component to use gop2p is the [`node.Node`](noe/node.go) struct, that contains the required _parameters_ (to **listen for messages, handle errors and connect or disconnect**) and _functions_ (to **create, start, wait or stop**) a `node.Node`:
